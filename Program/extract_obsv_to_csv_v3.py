...
# -*- coding: utf-8 -*-
"""
ArduPilot AP_Observerの新しいOBSVログフォーマット対応
TimeUS, PLX, PLY, PLZ, AX, AY, BX, BY, CX, CY, F, P, X, Y を抽出してCSV化
    ...
    while True:
        ...
        if msg_type == 'OBSV':
            try:
                ...
                # デバッグ出力（最初の3件のみ）
                if msg_count <= 3:
                    print(f"[DEBUG] レコード {msg_count}: TimeUS={record['TimeUS']}, PLX={record['PLX']:.6f}, PLY={record['PLY']:.6f}, PLZ={record['PLZ']:.6f}")
